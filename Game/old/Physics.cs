    ...
    public void Update(Character character) {
        if (character.Velocity.Z > 0) {
            if (character.Velocity.X != 0) {
                character.body.Position.X += character.Velocity.X * character.facing;
            }

            character.body.Position.Y = this.GetTrajectory(
                start_Y: character.body.Position.Y, 
                target_Y: character.floorLine, 
                max_Y: character.body.Position.Y - (int) character.Velocity.Y * 2, 
